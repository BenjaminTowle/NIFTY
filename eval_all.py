...
def _main(args, indices):
    ...
    global sample
    if sample is None:
        sample = dataset[0]["input_ids"].clone()
    else:
        assert (dataset[0]["input_ids"] == sample).all().item()
...
def main():
    ...
    alphas = [0.0, 0.5, 1.0, 2.0, 2.5]
    models = ["bb", "t5"]
    datasets = ["multiwoz", "dstc8"]

    idxs = list(range(5209))
    shuffle(idxs)
    indices = idxs[:100]

    for i, (a, m, d) in enumerate(product(alphas, models, datasets)):
        print(i)
        global dataset
        global model_type
        dataset = d
